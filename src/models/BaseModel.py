...
class GeneralModel(BaseModel):
	...
	class Dataset(BaseModel.Dataset):
		def _get_feed_dict(self, index):
			...
			if self.model.test_all and self.phase != 'train': 
...
class SAETuningDataset(BaseModel.Dataset):
    def __init__(self, model, corpus, phase: str):
        super().__init__(model, corpus, phase)
        self.user_vectors_cache = None
        self.item_ids_cache = None

    def prepare(self):
        logging.info(f"Pre-computing user embeddings for '{self.phase}' set to enable fast tuning...")
        
        class PrecomputationDataset(SequentialModel.Dataset):
            def _get_feed_dict(self, index: int) -> dict:
                user_id = self.data['user_id'][index]
                pos = self.data['position'][index]
                user_seq = self.corpus.user_his[user_id][:pos]
                if self.model.history_max > 0:
                    user_seq = user_seq[-self.model.history_max:]
                
                feed_dict = {
                    'user_id': user_id,
                    'item_id': np.array([self.data['item_id'][index]]),
                    'history_items': np.array([x[0] for x in user_seq]),
                    'history_times': np.array([x[1] for x in user_seq]),
                    'lengths': len(user_seq)
                }
                return feed_dict
        ...
        temp_dataset = PrecomputationDataset(self.model, self.corpus, self.phase)
        temp_dl = DataLoader(temp_dataset, batch_size=self.model.args.eval_batch_size, shuffle=False,
                             collate_fn=temp_dataset.collate_batch, pin_memory=self.model.args.pin_memory)
        ...
        self.model.eval() 
        with torch.no_grad():
            for batch in tqdm(temp_dl, desc=f"Pre-computing cache for {self.phase}"):
                batch_gpu = utils.batch_to_gpu(batch, self.model.device)
                
                user_vectors = self.model.get_sasrec_user_embedding(batch_gpu)
                
                all_user_vectors.append(user_vectors.cpu())
                all_item_ids.append(batch['item_id'][:, 0].cpu())
        self.user_vectors_cache = torch.cat(all_user_vectors, dim=0)
        self.item_ids_cache = torch.cat(all_item_ids, dim=0)

        logging.info(f"Pre-computation finished. Cached {len(self)} instances for '{self.phase}'.")

    def __len__(self):
        return len(self.item_ids_cache) if self.item_ids_cache is not None else 0

    def __getitem__(self, index: int) -> dict:
        if self.user_vectors_cache is None:
            raise RuntimeError("Dataset cache not prepared. Please call prepare() before accessing items.")
        return {
            'user_vectors_x': self.user_vectors_cache[index],
            'item_id': self.item_ids_cache[index] 
        }
    
    def collate_batch(self, feed_dicts: List[dict]) -> dict:
        return {
            'user_vectors_x': torch.stack([d['user_vectors_x'] for d in feed_dicts]),
            'item_id': torch.stack([d['item_id'] for d in feed_dicts])
        }
